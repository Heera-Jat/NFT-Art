...
function Liquidity () {
    ...
    const addLiquidity = async () => {
        if(selectedCount > 0 && fluidAmount > 0 && parseFloat(fluidAmount) <= parseFloat(fluidBalance)) {
            const provider = new ethers.JsonRpcProvider(NFT_CONTRACTS[chain.id].rpc);
            const nftContract = new ethers.Contract(NFT_CONTRACTS[chain.id][nftID].contractAddress, NFTABI, provider);
            const fluidERC20Contract = new ethers.Contract(NFT_CONTRACTS[chain.id].fluidAddress, FluidERC20ABI, provider);
            ...
            try {
                if(tokenAllowance < fluidAmount) {
                    await signedFluidERC20Contract.approve(NFT_CONTRACTS[chain.id].fluidity, ethers.parseUnits(fluidAmount, "ether"))
                }
                ...
                await signedFluidityContract.addLiquidity(NFT_CONTRACTS[chain.id][nftID].contractAddress, fluidNFTList, ethers.parseUnits(fluidAmount, "ether"))
            } catch (e) {

            }
        } else {
            console.log(typeof fluidAmount)
        }
    }
            ...
            <div className="w-full space-y-5"> 
                ...
                <div className="w-full space-y-1">
                    ...
                    <div className="flex justify-between bg-[rgb(14,19,32,0.3)] w-full h-[70px] rounded-[12px] p-4">
                        ...
                        <input onChange={(e) => setFluidAmount(e.target.value)} className="w-20 bg-[rgb(14,19,32,0.3)]" placeholder="0.00"/>
                    </div>
